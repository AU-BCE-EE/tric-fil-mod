...
plt.savefig('Ex 6 velocity comparison residuals.png', bbox_inches='tight')
plt.close()


#pH comparison

ex5 = pd.read_csv('Output data/residuals_delay_5.1.csv', sep = ',')
ex5['Moving_Average'] = ex5['residuals[g/m3]'].rolling(window=window_size,center = True).mean()

ex6 = pd.read_csv('Output data/residuals_delay_5.3.csv', sep = ',')
ex6['Moving_Average'] = ex6['residuals[g/m3]'].rolling(window=window_size,center = True).mean()

ex7 = pd.read_csv('Output data/residuals_delay_5.4.csv', sep = ',')
ex7['Moving_Average'] = ex7['residuals[g/m3]'].rolling(window=window_size,center = True).mean()

ex8 = pd.read_csv('Output data/residuals_delay_5.5.csv', sep = ',')
ex8['Moving_Average'] = ex8['residuals[g/m3]'].rolling(window=window_size,center = True).mean()

ex9 = pd.read_csv('Output data/residuals_delay_5.7.csv', sep = ',')
...
plt.savefig('Ex 5 pH comparison residuals.png', bbox_inches='tight')
