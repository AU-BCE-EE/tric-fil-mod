...
def rates(t, mc, v_g, v_l, cgin, clin, vol_gas, vol_liq, vol_tot, k, Kga, disp_gas, disp_liq, Daw, dx, counter = False, recirc = False):
...
# Model function
def tfmod(L, por_g, por_l, v_g, v_l, nc, cg0, cl0, cgin, clin, Kga, k, henry, pKa, pH, temp, dens_l, times, pres = 1., ssa = 1100, counter = False, recirc = False, disp_gas = 0., disp_liq = 0.):
   ...
   # disp_* = effective gas- or liquid-phase dispersion coefficient (m2/s)
   ...
   out = solve_ivp(rates, [0, max(times)], y0 = y0, 
                   t_eval = times, 
                   args = (v_g, v_l, cgin, clin, vol_gas, vol_liq, vol_tot, k, Kga, disp_gas, disp_liq, Daw, dx, counter, recirc),
                   method = 'LSODA')
