...
por_g = 0.76      # (m3/m3) Estimated by porosity experiments
por_l = 0.23     # (m3/m3) Estimated by porosity experiments
...
#First inlet profile (first data obtained)
ex1=Data_reciever(filename='experiment_2.4.inlet1.csv',startcycle=45,stopcycle=1145,a=constant1,b=constant2
                  )
#second inlet profile (last data obtained)
ex5=Data_reciever(filename='experiment_2.4.inlet2.csv',startcycle=69,stopcycle=1169,a=constant1,b=constant2)
...
pH1=8.53
pred1 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = 53/3600, v_l =1.2/3600, nc = nc, cg0 = cg0, 
              cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
              pH = pH1, temp = temp, dens_l = dens_l, times = times)
pred1label= '2.4.1 model' #label on plots


pH2=8.54

pred2 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = 53/3600, v_l = 1.2/3600, nc = nc, cg0 = cg0, 
              cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
              pH = pH2, temp = temp, dens_l = dens_l, times = times)
pred2label='2.4.2 model'

pH3=8.25

pred3 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = 53/3600, v_l = 1.2/3600, nc = nc, cg0 = cg0, 
              cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
              pH = pH3, temp = temp, dens_l = dens_l, times = times)
pred3label='2.4.3 model'
...
ex2=Data_reciever(filename='experiment_2.4.1.csv',startcycle=39,stopcycle=1100,a=constant1,b=constant2)
ex3=Data_reciever(filename='experiment_2.4.2.csv',startcycle=32,stopcycle=1100,a=constant1,b=constant2)
ex4=Data_reciever(filename='experiment_2.4.3.csv',startcycle=82,stopcycle=1100,a=constant1,b=constant2)

#plotting


plt.plot(ex2['time'],ex2['concentration'],label='2.4.1 experimental data')
plt.plot(ex3['time'],ex3['concentration'],label='2.4.2 experimental data')
plt.plot(ex4['time'],ex4['concentration'],label='2.4.3 experimental data')
