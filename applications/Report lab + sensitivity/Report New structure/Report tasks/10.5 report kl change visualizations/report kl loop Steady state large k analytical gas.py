...
Kga_list = [0.001,0.008,0.02,'onda',0.1] 
...
for i, Kga in enumerate(Kga_list):
    ...
    pred = tfmod(L=L, por_g=por_g, por_l=por_l, v_g=v_g, v_l=v_l, nc=nc, cg0=cg0, cl0=cl0,
                 cgin=cgin, clin=0, Kga=Kga, k=k, k2=k, henry=henry, pKa=pKa,
    ...
    label = f"Kga {Kga} m/s outlet gas phase"+extra
    
    # Closed-form solution ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    z = L
    cg = np.array(c_in) * np.exp(-Kga / v_g * z)
    
    dev = (pred['gas_conc'][nc - 1, :][199]-cg)/cg * 100
    
    print(label)
    print('analytical  '+str(cg))
    print('model  '+str(pred['gas_conc'][nc - 1, :][199]))
    print ('RE  '+str(formatted_RE))
