...
for i in range(1,8):
    ...
    pred1 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = v_g, v_l = v_l, nc = nc, cg0 = cg0, 
                  ...
                  pH = pH1, temp = temp, dens_l = dens_l, times = times, v_res = 0, recirc = False, counter = False)
    pred1label= 'recirc = off, counter = off, v_res = off'
    ...
    plt.title('Gas Phase '+pred1label)
    plt.legend()
    plt.subplot(111).legend(loc='upper center',bbox_to_anchor=(0.5,-0.2)) #Moves legend out of plot
    plt.savefig('Plots/none/Experiment '+first+'.'+second+' gasprofile.png', bbox_inches='tight')
    plt.close()
    ...
    plt.xlabel('Location (m)')
    plt.ylabel('Compound conc. (g/m3)')
    plt.title('Liquid Phase '+pred1label)
    plt.legend()
    plt.subplot(111).legend(loc='upper center',bbox_to_anchor=(0.5,-0.2)) #Moves legend out of plot
    plt.savefig('Plots/none/Experiment '+first+'.'+second+' liqprofile.png', bbox_inches='tight')
    plt.close()
    
    from tabulate import tabulate
    parameters = [['v_g', pred1['inputs']['v_g']], ['v_l', pred1['inputs']['v_l']], ['pH1', pH1], ['k', k],
                   ['water content', por_l], ['porosity', por_g], ['temperature', temp], ['kl',pred1['pars']['kl']],['kg',pred1['pars']['kg']],['ae',pred1['pars']['ae']]]
