...
k=0
ae_list = np.linspace(500.0, 1500.0,5) 
...
for i, ae in enumerate(ae_list):
    pred = tfmod(L=L, por_g=por_g, por_l=por_l, v_g=v_g, v_l=v_l, nc=nc, cg0=cg0, cl0=cl0,
                 ...
                 pH=pH1, temp=temp, dens_l=dens_l, times=times, v_res=v_res, kg = 'onda', kl = 'onda', ae=ae, 
                 recirc=True, counter=True)
    
    label = f"{first}.{second}.ae {ae} model"
...
plt.savefig('Plots/Experiment '+first+'.'+second+'ae loop.png', bbox_inches='tight')
plt.close()
