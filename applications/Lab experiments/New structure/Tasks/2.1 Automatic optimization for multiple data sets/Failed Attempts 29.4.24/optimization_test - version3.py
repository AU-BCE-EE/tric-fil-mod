...
for i in [exp_no1,exp_no2]:
        ...
        #Removing all noise by taking the average of 50 data points
        t1norm = ex1['Time in h'] - ex1['Time in h'][cycle1]
        t1 = t1norm [cycle1+(int(window_size/2)):cycle1+length-int((window_size/2))]
        C1= ex1['Concentration in g/m^3'].rolling(window=window_size,center = True).mean()[cycle1+(int(window_size/2)):cycle1+length-int((window_size/2))]

        t5norm = ex5['Time in h'] - ex5['Time in h'][cycle5]
        t5 = t5norm [cycle5+int(window_size/2):cycle5+length-int((window_size/2))]
        C5= ex5['Concentration in g/m^3'].rolling(window=window_size,center = True).mean()[cycle5+int(window_size/2):cycle5+length-int((window_size/2))]

        t2norm = ex2['Time in h'] - ex2['Time in h'][cycle2]
        t2 = t2norm [cycle2+int(window_size/2):cycle2+length-int(window_size/2)]
        C2= ex2['Concentration in g/m^3'].rolling(window=window_size,center = True).mean()[cycle2+int(window_size/2):cycle2+length-int(window_size/2)]

        t3norm = ex3['Time in h'] - ex3['Time in h'][cycle3]
        t3 = t3norm [cycle3+int(window_size/2):cycle3+length-int(window_size/2)]
        C3= ex3['Concentration in g/m^3'].rolling(window=window_size,center = True).mean()[cycle3+int(window_size/2):cycle3+length-int(window_size/2)]
...
#Transforming the data, so that the concentrations are larger, by multiplying by 10.000
times2 = results_dict['experimental5.'+str(exp_no2)]['experimental time (h)'] *3600
c_meas2 = 10000 * results_dict['experimental5.'+str(exp_no2)]['experimental_out'] 
c_in2 = 10000 * results_dict['experimental5.'+str(exp_no2)]['experimental_in']
cgin2 = pd.DataFrame({'time': times2, 
                     'cgin': c_in2})

times1 = results_dict['experimental5.'+str(exp_no1)]['experimental time (h)'] *3600
c_meas1 = 10000 * results_dict['experimental5.'+str(exp_no1)]['experimental_out'] 
c_in1 = 10000 * results_dict['experimental5.'+str(exp_no1)]['experimental_in']
...
sol = least_squares(res_calc, [IG_kl, IG_k, IG_k2],bounds = (0 , 1), diff_step = diff_step, xtol = 3e-16)
...
plt.savefig('Experiment '+first+'.'+second+'optimized, no noise, 10000, [IG] ['+str(IG_kl)+','+str(IG_k)+','+str(IG_k2)+'].png', bbox_inches='tight')
plt.close()
...
# Save the figure
plt.savefig('Input_parameters_optimized, no noise, 10000 [IG] ['+str(IG_kl)+','+str(IG_k)+','+str(IG_k2)+'].png', bbox_inches='tight')
