...
for i, kl in enumerate(kl_list):
    pred = tfmod(L=L, por_g=por_g, por_l=por_l, v_g=v_g, v_l=v_l, nc=nc, cg0=cg0, cl0=cl0,
                 ...
                 pH=pH, temp=temp, dens_l=dens_l, times=times, v_res=v_res, kg = 'onda', kl = kl, ae=800, 
                 recirc=True, counter=True)
    
    
    label = f"kl {kl} m/s outlet liquid phase"
    ...
    z = L
    cg = np.array(c_in) * np.exp(-Kga / v_g * z)
    cl = cg / Daw
    analytical.append(cl)
...
for i, (pred, label) in enumerate(zip(preds, pred_labels)):
    plt.plot(pred['time'] / 3600, pred['liq_conc'][nc - 1, :], label=label+' model',color=colors[i % len(colors)])
for i, (pred, label) in enumerate(zip(preds, pred_labels)):
...
plt.savefig('Plots/k steady state kl 1 + analytical liq.png', bbox_inches='tight')
plt.close()
