...
for i, kl in enumerate(kl_list):
    pred = tfmod(L=L, por_g=por_g, por_l=por_l, v_g=v_g, v_l=v_l, nc=nc, cg0=cg0, cl0=cl0,
                 ...
                 pH=pH, temp=temp, dens_l=dens_l, times=times, v_res=v_res, kg = 'onda', kl = kl, ae='onda', k2=k, 
    ...
    if i == 1:
        ...
        extra = ''
    label = f"kl = {kl:.3g} m/s outlet"+extra
...
plt.title('Liquid mass transfer coefficient variation')
plt.savefig('Plots/Kl variation for report.png', bbox_inches='tight')
plt.close()
