...
# minlen = min(len(ex4['Moving_Average']),len(ex8['Moving_Average']))
# MAE = minlen**-1*np.sum(abs(ex4['Moving_Average'][:minlen]-ex8['Moving_Average'][:minlen]))
# print(MAE)
...
#plt.plot(ex4['experimental time (h)'][:minlen]*60,ex8['Moving_Average'][:minlen]-ex4['Moving_Average'][:minlen],label = 'residuals')
...
plt.plot(ex4['experimental time (h)']*60, expected_inlet, color='g', label='Nominal inlet concentration')
plt.xlabel('Time (min)')
plt.ylabel('Compound conc. (g/m3)')
plt.legend()
plt.grid(True)
