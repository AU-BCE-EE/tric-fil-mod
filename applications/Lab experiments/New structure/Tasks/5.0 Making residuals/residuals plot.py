...
k2_list = [0, 0.001,0.008,0.01 ,0.025]
...
for i, k2 in enumerate(k2_list):
    ...
    label = f"{first}.{second}.k2 {k2} model"
...
plt.plot(t3,c_out,label='measured outlet')
...
#Plotting residuals
r = np.transpose(c_out - pred1['gas_conc'][nc-1:])

    

plt.clf()
plt.plot(t3,r,label=pred1label)
for pred, label in zip(preds, pred_labels):
