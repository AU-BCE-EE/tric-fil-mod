...
for i, temp in enumerate(temp_list):
    ...
    label = f"{first}.{second}.temp {temp} model"
...
for pred, label in zip(preds, pred_labels):
    plt.plot(pred['time'][:176] / 3600, pred['gas_conc'][nc - 1, :], label=label)
plt.plot(pred1['time'][:176] / 3600, pred1['gas_conc'][nc - 1, :],label=pred1label)
plt.xlabel('Time (h)')
