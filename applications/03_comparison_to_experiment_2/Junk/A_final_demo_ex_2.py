...
#comparison to data experiment 2.1
...
plt.axvline(x=BT1/60,linestyle='-',label=BT1label)
...
#               cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
#               pH = pH, temp = temp, dens_l = dens_l, times = times)
# pred3label='2. 3v_g=106m/h, v_l=1.2m/h'
...
#               cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
#               pH = pH, temp = temp, dens_l = dens_l, times = times)
# pred4label='2.4 v_g=53m/h, v_l=1.2m/h'


# pred5 = tfmod(L = L, por_g = por_g, por_l = por_l,v_g = 79/3600, v_l = 1.2/3600, nc = nc, cg0 = cg0, 
#               cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
#               pH = pH, temp = temp, dens_l = dens_l, times = times)
# pred5label='2.5 v_g=79m/h, v_l=1.2m/h'


# pred6 = tfmod(L = L, por_g = por_g, por_l = por_l,v_g = 79/3600, v_l = 0.4/3600, nc = nc, cg0 = cg0, 
#               cl0 = cl0, cgin = cgin, clin = clin, Kga = 'onda', k = k, henry = henry, pKa = pKa, 
#               pH = pH, temp = temp, dens_l = dens_l, times = times)
# pred6label='2.6v_g=79m/h, v_l=0.4m/h'
...
# plt.axvline(x=BT1/60,color='violet',linestyle='-',label=BT1label)
# plt.axvline(x=BT2/60,color='deeppink',linestyle='-',label=BT2label)
# plt.axvline(x=BT3/60,color='purple',linestyle='-',label=BT3label)
...
# plt.axvline(x=BT1/60,color='violet',linestyle='-',label=BT1label)
# plt.axvline(x=BT3/60,color='purple',linestyle='-',label=BT3label)
# plt.axhline(y=cgin/Daw,color='g', linestyle='dashed',label='equilibrium with inlet concentration')
# plt.xlabel('Time (h)')
# plt.ylabel('Compound conc. (g/m3)')
# plt.subplot(111).legend(loc='upper center',bbox_to_anchor=(0.5,-0.5))
...
# plt.plot(t1,C_out1,color='b', label=pred1label)
# plt.plot(t2,C_out2,color='r',label=pred2label)
# plt.plot(t3,C_out3,color='y',label=pred3label)
# plt.plot(t4,C_out4,color='k',label=pred4label)
# plt.plot(t5,C_out5,color='c',label=pred5label)
# plt.plot(t6,C_out6,color='m',label=pred6label)
# plt.axvline(x=BT1/60,color='violet',linestyle='-',label=BT1label)
# plt.axvline(x=BT2/60,color='deeppink',linestyle='-',label=BT2label)
# plt.axvline(x=BT3/60,color='purple',linestyle='-',label=BT3label)
# plt.axhline(y=cgin,color='g',label='inlet concentration')
# plt.subplot(111).legend(loc='upper center',bbox_to_anchor=(0.5,-0.5))
# plt.title('Experiment 2 all experimental data')
# plt.xlabel('Time(h)')
