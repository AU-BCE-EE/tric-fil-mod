...
if not cycle4 == 'NaN':
     ...
     C2_cut = ex2['Concentration in g/m^3'].rolling(window=5,center = True).mean()[cycle2:cycle2+minlen]
     C3_cut = ex3['Concentration in g/m^3'].rolling(window=5,center = True).mean()[cycle3:cycle3+minlen]
     c_out = np.mean([C2_cut,C3_cut][:minlen],axis=0)
...
k_list = [0.003]
#k_list = [0, 0.001,0.008,0.01 ,0.025]
...
for i, k in enumerate(k_list):
    ...
    for j, kl in enumerate(kl_list):
        ...
        #Plotting
        plt.clf()
        plt.plot(t2,c_out,label='Measured outlet')
        plt.plot(t1,c_in,label='Measured inlet')
        plt.plot(pred['time'] / 3600, pred['gas_conc'][nc - 1, :],color='k',label='Model')
        ...
        plt.savefig('Plots/Experiment '+label+str(loop)+'.png', bbox_inches='tight')
        plt.show()
        plt.close()
        ...
        plt.plot(pred['time'] / 3600, r, label=label)
        plt.xlabel('Time (h)')
        ...
        plt.savefig('Plots/Experiment '+label+str(loop)+'residuals.png', bbox_inches='tight')
        plt.close()
        ...
        parameters = [['v_g [m/s]', "{:.4f}".format (pred['inputs']['v_g'])], ['v_l [m/s]',"{:.6f}".format (pred['inputs']['v_l'])], ['pH1', "{:.2f}".format(pH1)], ['pH2', "{:.2f}".format(pH2)], ['k [1/s]', "{:.3f}".format(k)], ['countercurrent', pred['inputs']['counter']],['Kga [1/s]', pred['pars']['Kga']],
                      ['recirculation', pred['inputs']['recirc']], ['water content [m^3/m^3]', "{:.2f}".format(por_l)], ['porosity [m^3/m^3]', "{:.2f}".format(por_g)], ['temperature [deg. C]', "{:.0f}".format(temp)], ['v_res [m^3/m^2]', "{:.4f}".format(pred['inputs']['v_res'])],['kl', kl]]
        ...
        # Save the figure
        plt.savefig('Plots/Inputs/Input_parameters_'+label+str(loop)+'.png', bbox_inches='tight')
        ...
        # Gas
        plt.clf()
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 1], label = 'Time:%1.0f'%times[1]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 20], label = 'Time:%1.0f'%times[20]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 40], label = 'Time:%1.0f'%times[40]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 60], label = 'Time:%1.0f'%times[60]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 80], label = 'Time:%1.0f'%times[80]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 100], label = 'Time:%1.0f'%times[100]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 120], label = 'Time:%1.0f'%times[120]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 160], label = 'Time:%1.0f'%times[160]+'s')
        plt.plot(pred['cell_pos'], pred['gas_conc'][:, 199], label = 'Time:%1.0f'%times[199]+'s')
        plt.xlabel('Location (m)')
        plt.ylabel('Compound conc. (g/m3)')
        plt.title('Gas Phase')
        plt.legend()
        plt.subplot(111).legend(loc='upper center',bbox_to_anchor=(0.5,-0.2)) #Moves legend out of plot
        plt.savefig('Plots/Experiment '+label+str(loop)+' gasprofile.png', bbox_inches='tight')
        plt.close()


        #Liquid
        plt.clf()
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 1], label = 'Time:%1.0f'%times[1]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 20], label = 'Time:%1.0f'%times[20]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 40], label = 'Time:%1.0f'%times[40]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 60], label = 'Time:%1.0f'%times[60]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 80], label = 'Time:%1.0f'%times[80]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 100], label = 'Time:%1.0f'%times[100]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 120], label = 'Time:%1.0f'%times[120]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 160], label = 'Time:%1.0f'%times[160]+'s')
        plt.plot(pred['cell_pos'], pred['liq_conc'][:, 199], label = 'Time:%1.0f'%times[199]+'s')
        plt.xlabel('Location (m)')
        plt.ylabel('Compound conc. (g/m3)')
        plt.title('Liquid Phase')
        plt.legend()
        plt.subplot(111).legend(loc='upper center',bbox_to_anchor=(0.5,-0.2)) #Moves legend out of plot
        plt.savefig('Plots/Experiment '+label+str(loop)+' liqprofile.png', bbox_inches='tight')
        plt.close()
