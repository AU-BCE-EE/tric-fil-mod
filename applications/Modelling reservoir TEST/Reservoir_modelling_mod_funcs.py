...
def rates(t, mc, v_g, v_l, cgin, clin, vol_gas, vol_liq, vol_tot, k, Kga, Daw, tt, nt, v_res , counter = True, recirc = True):
  ...
  if recirc:
      if counter:
          ...
          oi = nc - 1
  # Modelling of a reservoir with volume v_res into which the liquid poured and drawn from, if recirc is true. 
      if v_res >0:
          c_res = clin
          clin = ((tt / nt *3600 * v_l * ccl[oi]) + ( c_res * v_res)) / v_res
      else:
          clin = ccl[oi]
...
# Model function
def tfmod(L, por_g, por_l, v_g, v_l, nc, cg0, cl0, cgin, clin, Kga, k, henry, pKa, pH, temp, dens_l, times, tt, nt, v_res, pres = 1., ssa = 1100, counter = True, recirc = True):
