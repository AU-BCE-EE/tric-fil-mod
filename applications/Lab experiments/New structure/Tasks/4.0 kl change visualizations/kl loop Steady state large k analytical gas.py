...
for i, kl in enumerate(kl_list):
    ...
    label = f"kl {kl} m/s outlet gas phase"
...
plt.clf()
for i, (pred, label, analytic) in enumerate(zip(preds, pred_labels, analytical)):
    plt.plot(pred['time'] / 3600, pred['gas_conc'][nc - 1, :], label=label + ' model', color=colors[i % len(colors)])
    plt.plot(pred['time'] / 3600, analytic, label=label + ' analytical solution', color=colors[i % len(colors)], linestyle='dashed')
plt.xlabel('Time (h)')
