...
plt.plot(t1 * 60, expected_inlet, color='g', label='Nominal inlet concentration')
plt.xlabel('Time (min)')
...
plt.savefig('..//Plots for report/Raw experimental data 4.1.png', bbox_inches='tight')
plt.close()
...
C4= ex4['Concentration in g/m^3'][cycle4:cycle4+length]
...
MAE_12 = float(length)**-1*np.sum(abs(C2-C3))

MAE_13 = float(length)**-1*np.sum(abs(C2-C4))

MAE_23 = float(length)**-1*np.sum(abs(C4-C3))

print('rep1 compared to rep2 '+str(MAE_12))
print('rep1 compared to rep3 '+str(MAE_13))
print('rep2 compared to rep3 '+str(MAE_23))
