...
for a in range (6,7):
    ...
    for i in range (1,no_exp):
        ...
        pred1 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = v_g, v_l = v_l, nc = nc, cg0 = cg0, 
                      ...
                      pH = pH1, temp = temp, dens_l = dens_l, times = times, v_res = v_res, k2=k, kl = 0.000001,recirc = recirc, counter = True)
        pred1label= first+'.'+second+'.1 model' #label on
        
        pred2 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = v_g, v_l = v_l, nc = nc, cg0 = cg0, 
                      cl0 = cl0, cgin = cgin, clin = clin, Kga = 'individual', k = k, henry = henry, pKa = pKa, 
                      pH = pH2, temp = temp, dens_l = dens_l, times = times, v_res = v_res, k2 = k, kl = 0.000001, recirc = recirc, counter = True)
        pred2label= first+'.'+second+'.2 model' #label on
        
        if not cycle4 == 'NaN':
           pred3 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = v_g, v_l = v_l, nc = nc, cg0 = cg0, 
                         cl0 = cl0, cgin = cgin, clin = clin, Kga = 'individual', k = k, henry = henry, pKa = pKa, 
                         pH = pH3, temp = temp, dens_l = dens_l, times = times, v_res = v_res, k2 = k,kl = 0.000001, recirc = recirc, counter = True)
