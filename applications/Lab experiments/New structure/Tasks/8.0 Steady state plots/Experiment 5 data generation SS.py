...
for a in range (5,6):
    ...
    for i in range (1,no_exp):
        ...
        #Setting concentration for steady state
       ...
       # Number of time rows
        ...
        results = pd.DataFrame ({'experimental':c_out[10:1410] , 'experimental time (h)':t3norm [cycle3:cycle3+1400] })
        ...
        results = pd.DataFrame ({'model':mod_out[0:1400] , 'model time(h)':pred1['time'][0:1400] / 3600 })
        ...
        r = np.transpose(c_out[10:1410] - pred['gas_conc'][nc - 1, :][0:1400])
        
        results = pd.DataFrame({'residuals[g/m3]':r, 'time_residuals[h]':t3norm[cycle3:cycle3+1400]})
