...
pred1 = tfmod(L = L, por_g = por_g, por_l = por_l, v_g = v_g, v_l = v_l, nc = nc, cg0 = cg0, 
              cl0 = cl0, cgin = cgin, clin = clin, Kga = 'individual', k = k, k2 = k, henry = henry, pKa = pKa, 
              pH = pH1, temp = temp, dens_l = dens_l, times = times, v_res = v_res, kl = 'onda', ae=800, recirc = False, counter = True)
...
for pred, label in zip(preds, pred_labels):
    plt.plot(pred['time'] / 3600, pred['liq_conc'][nc - 1, :], label=label)
plt.plot(pred1['time'] / 3600, pred1['liq_conc'][nc - 1, :],label=pred1label)
plt.xlabel('Time (h)')
...
plt.savefig('Plots/Experiment '+first+'.'+second+'k steady state kl onda liq.png', bbox_inches='tight')
plt.close()
